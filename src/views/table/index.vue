...
<!--            >-->
<!--              <el-input-->
<!--                v-model="dataModel[item.FieldName]"-->
...
<!--            </el-form-item>-->
            <el-form-item :label="item.FieldComment" v-for="(item,index) in title" :key="item.FieldName">
              ...
              <el-input v-else v-model="dataModel[item.FieldName]"  :placeholder="item.FieldType" type="file"> </el-input>
              <!-- <el-upload
                v-else
                class="upload"
...
export default {
  ...
  methods: {
    ...
    // },
    before_Upload(file) {
      fd.append(e.FieldName, file);
    },
    onSubmit() {
      let fd = new FormData();
      this.title.forEach((e) => {
        console.log(
          "key:::" + e.FieldName + " ,value::::" + this.dataModel[e.FieldName]
        );
      ...
      insert(this.tabId, fd)
        .then((res) => {
          if (res) {
            ...
            this.$router.push({ path: "#/detail/table" }); //跳转到首页
          }
        })
        .catch((error) => {
          console.log(error);
          this.$router.push({ path: "#/detail/table" });
        });
    ...
    download(item, scope) {
      ...
      getObject(this.tabId, parmas.id, parmas.fieldName)
        .then((res) => {
          //
